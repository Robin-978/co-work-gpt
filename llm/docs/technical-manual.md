...
模板內有四個佔位符，由 `build.js` 置換：

| 佔位符 | 內容 |
|---|---|
| `__WEIGHTS__` | `weights.json` 全文 |
| `__CORPUS_CHARS__` | `buildCorpus().length` 的千分位字串 |
| `__DOC_USER__` | `docs/user-manual.md` 轉成的 HTML |
| `__DOC_TECH__` | `docs/technical-manual.md` 轉成的 HTML |

**兩份手冊的唯一真相是 `docs/*.md`。** 頁面最下面的「附錄：手冊」是建置時從那兩個檔案轉進來的，不是手抄的副本 — 改手冊只改 `.md`，重跑 `build.js` 就同步了。
...
代入：`64,512 + 8,192 + 2×41,728 + 128 = 156,288`。
...
```jsonc
{
  "cfg":   { "V":1008, "C":64, "L":2, "H":4, "F":192, "B":128 },
...
- `matmul` 是樸素三層迴圈，但跳過 `a === 0` 的項。序列最長 128、`C=64`，這個規模不需要分塊或 SIMD。
...
1. 讀 `weights.json` 與 `page.template.html`，用 `corpus.js` 算語料字數，用 `md.js` 把兩份手冊轉成 HTML。
2. **先檢查**要內嵌的東西裡沒有 `</script>`（權重與手冊都檢查；有的話會提早關掉 `<script>` 標籤，整頁壞掉）。
3. **先在模板上確認**每個佔位符剛好出現一次，然後才置換（用函式形式的 `replace`，避免 `$&` 之類的替換樣式被誤解讀）。
4. 寫檔，印出大小。

> 第 3 步刻意是「置換前數一次」而不是「置換後檢查殘留」。技術手冊本文就在講 `__WEIGHTS__` 這些佔位符，那些字串會合法地出現在產出裡 — 用殘留檢查會誤判成置換失敗。這個坑是真的踩過才改的。

### `md.js`：極小的 Markdown 轉換器

只支援 `docs/*.md` 實際用到的語法：標題、段落、清單、引用、水平線、表格（含 `---:` 靠右）、程式碼區塊與行內程式碼、粗體、連結。刻意不做巢狀清單與圖片。

三個實作上的重點：

- **行內程式碼先抽走**再做其他規則，否則 `` `<eos>` ``、`` `</script>` `` 這種內容會被當成 HTML。佔位符用 `\u0000` 包住 — 一開始用「空白＋數字＋空白」，結果把正文裡的數字吃掉了。
- **連結重寫**：手冊互相引用（`user-manual.md` ↔ `technical-manual.md`）在頁內變成 `#doc-user` / `#doc-tech` 錨點；指向 repo 檔案的（`../README.md`）降成純文字，因為單檔網頁連不到。對照表在 `build.js` 的 `LINKS`。
- **`assertNoMarkdown()`** 在轉完之後掃一遍產出，還看得到 `**`、`|` 開頭的行、`#` 標題、`[](…)` 就丟錯。加了沒支援的語法會讓 build 失敗，而不是默默出一頁壞掉的手冊。

表格一律包一層 `.mdTableWrap`（`overflow-x:auto`），否則手冊裡的寬表格在手機寬度會撐爆版面。
...
| 拉長上下文 | `BLOCK=` 重訓。`RESUME` 可以拉長（舊位置照抄、新位置隨機初始化），但字典同時大幅變動時反而比從零差 — 見下方 |
| 試超參數 / 量速度 | **一定要加 `OUT=`**。`run_train.js` 在 `step === STEPS` 也會存檔，`STEPS=50` 就足以洗掉訓練好的檢查點 |
...
| 改手冊內容 | 只改 `docs/*.md`，重跑 `build.js`。**不要改頁面裡那份** — 下次建置會被覆蓋 |
| 手冊要用新的 Markdown 語法 | 在 `md.js` 的 `render()` 加規則；沒加就會被 `assertNoMarkdown()` 擋下來 |

---

## 11. 上下文有沒有真的被用到

`B` 從 48 拉到 128 只有一個地方要動（`wpe` 的長度），但**拉長本身不會讓模型變好** — 只有語料裡真的存在跨過 48 個 token 的依賴，多出來的視窗才有東西可看。`ctx_test.js` 直接量這件事：對段落裡每個位置 ≥ 48 的字，分別用完整前綴和只留最後 N 個字去預測，比較 NLL。

```
  可見的前文長度        平均 NLL    困惑度
  只留最後   4 個 token     1.3750      3.96
  只留最後   8 個 token     0.3488      1.42
  只留最後  16 個 token     0.0420      1.04
  只留最後  24 個 token     0.0130      1.01
  只留最後  48 個 token     0.0075      1.01
  完整前綴（最多 128）       0.0080      1.01
```

**結論是負的：完整前綴沒有比只留 48 個好，資訊在 16～24 個字就飽和。** 這個語料只有 10 萬字、模型 15.6 萬參數，best loss 0.11 — 它把語料背下來了，而 16 個字已經足以認出是哪一段。

**為什麼掃一整排 N 而不是只比兩組：** 只比一組的話，「沒有差別」和「程式量錯了」看起來一模一樣。短的 N 必須看得出明顯劣化（只留 4 個字時 NLL 是 173 倍），那是這個實驗的對照組 — 先確認量法對「看不到前文」會叫，才有資格相信它說「沒差」。

要讓長上下文真的被用到，需要的是大到背不起來的語料，或刻意設計的長距離任務，而不是再把 `B` 往上調。
...
- **檔案大小** 約 308 KB：權重 JSON 約 211 KB、兩份手冊約 24 KB、其餘是模板本身。int8 量化把浮點權重壓成四分之一；再往下壓（int4）會開始看得出生成品質變差。
...
- **換模型之後**：`node ctx_test.js` 看上下文實際用到幾個字；`node probe.js 0.9` 看字典健康度與分布，並確認舊語料沒被洗掉。
- **內嵌手冊**：`node md.js --test` 全過；頁面上兩份手冊預設收起、標題連結與手冊內互相引用都會展開對應區塊、用 `#doc-tech` 開頁也會展開；展開後在 390px 仍無水平溢出（寬表格由 `.mdTableWrap` 自己捲）。
...
- [`llm/README.md`](../README.md) — 訓練、續訓、擴充字典、RAG、專家系統、生產資料工具鏈
